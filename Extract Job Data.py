...
def add_job_attributes_to_df(job_list, job_df_loop, job_id_dict, keyword_df_loop, keyword_id_dict, search_term):
    '''
    ...
    Returns this information as a pandas row. Returns information for both job and keyword dataframes. 
    '''    
    for job in job_list:
        ...
        try:
            if new_job is not None:
                job_df_loop = job_df_loop.append(new_job, verify_integrity = True)
        except ValueError:
            pass
        
        try:
            if new_keyword is not None:    
                keyword_df_loop = keyword_df_loop.append(new_keyword, verify_integrity = True)
        except ValueError:
            pass

    return job_df_loop, keyword_df_loop
...
def append_job_info(search_term, search_location, page_limit = 100):
    ...
    '''
    job_df_loop = create_initial_empty_df('job') 
    job_id_dict = get_searchable_dict('job') 
    
    keyword_df_loop = create_initial_empty_df('keyword')
    ...
    for page in range(1,page_limit+1):    
        ...
        job_df_loop, keyword_df_loop = add_job_attributes_to_df(job_list, job_df_loop, job_id_dict, keyword_df_loop, keyword_id_dict, search_term)
...
def get_job_location_type(job_df):
    '''
...
def get_job_location_attrs(job_df, country):
    '''Pass dataframe into various functions that parse out location attributes from the location string using method chaining. Parsed attributes are populated into the database.'''

    job_df = (
        job_df.pipe(get_job_location_type).apply(get_job_location_model, axis=1).apply(get_job_city, axis=1).apply(get_job_jurisdiction, axis=1).pipe(get_job_country, country)
    )

    return job_df[['job_title','company','location','rating_provided','rating','salary_provided','weblink','date_recorded','location_model','country', 'jurisdiction','city']]
...
def get_indeed_job_data(search_terms, country = 'Canada', page_limit = 30):
    for keyword in search_terms:
        job_df, keyword_df = append_job_info(keyword, country, page_limit)
        salary_df = create_salary_df(job_df)

        job_df = get_job_location_attrs(job_df, country)
        print(f'New {keyword} jobs found to be added: {len(job_df.index)}')

        append_pandas_to_sql(job_df, 'job')
